...
(function(){
  ...
  function addToMarket(item,persist=true){
    ...
    arr.push({index:idx,cls:item.age,name:`${item.a} & ${item.b}`,club:'',rank:(classRanks[item.age]||0)+1,price:Number(item.price)||6,form:0,recentPoints:[],priceChange:0,userAdded:true,competitionClass:item.level});
  ...
  function evaluate(){
    ...
    const level=document.getElementById('newPairLevel')?.value||'A';
    ...
    save.disabled=false; save.dataset.price=price; save.dataset.a=a;save.dataset.b=b;save.dataset.age=age;save.dataset.level=level;
  }
