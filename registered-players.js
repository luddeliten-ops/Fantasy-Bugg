...
(function(){
  ...
  function firstDeadlinePassed(){
    try{
      if(typeof firstFantasyDeadline==='function'){
        const first=firstFantasyDeadline();
        if(!first || !first.deadline) return false;
        return Date.now()>=new Date(first.deadline).getTime();
      }
      if(typeof hasFirstDeadlinePassed==='function') return !!hasFirstDeadlinePassed();
    }catch(e){}
    return false;
  }

  function showHiddenTeamMessage(){
  ...
  function openLeaderboardTeam(userId){
    if(!userId) return;
    const isOwn=typeof currentUser!=='undefined' && currentUser && String(userId)===String(currentUser.id);
    if(!isOwn && !firstDeadlinePassed()){
  ...
  function renderRegisteredLeaderboard(){
    ...
    if(!box) return;
    if(!cachedRows.length){box.innerHTML='<div class="empty">Topplistan är tom.</div>';return;}
    if(typeof globalRank!=='undefined') globalRank='–';
    cachedRows.forEach((row,index)=>{if(typeof currentUser!=='undefined'&&currentUser&&String(row.user_id)===String(currentUser.id)){try{globalRank=index+1;}catch(e){}}});
    ...
    box.querySelectorAll('[data-public-team]').forEach(button=>{button.onclick=()=>openLeaderboardTeam(button.dataset.publicTeam);});
    const toggle=box.querySelector('#toggleGlobalLeaderboard');
    if(toggle) toggle.onclick=()=>{leaderboardExpanded=!leaderboardExpanded;renderRegisteredLeaderboard();};
  ...
  async function refreshRegisteredCounts(){
    ...
    try{const profiles=await getProfiles();const count=profiles.length;const managerEl=document.getElementById('homeManagerCount');const teamEl=document.getElementById('homeTeamCount');if(managerEl)managerEl.textContent=count.toLocaleString('sv-SE');if(teamEl)teamEl.textContent=count.toLocaleString('sv-SE');}catch(error){console.error('Kunde inte räkna registrerade managers:',error);}
  }

  if(typeof window.loadGlobalLeaderboard==='function') window.__originalLoadGlobalLeaderboard=window.loadGlobalLeaderboard;
  window.loadGlobalLeaderboard=loadAllRegisteredLeaderboard;
  async function run(){await refreshRegisteredCounts();await loadAllRegisteredLeaderboard();}
